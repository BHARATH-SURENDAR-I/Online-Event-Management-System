...
$cmd = isset($_GET['cmd']) ? $_GET['cmd'] : 'create';
switch($cmd) {
	case 'create':
		createUser();
	break;
...
function createUser() {
	...
	dbQuery($sql);
	$bodymsg = "User $name booked the date slot on $bkdate. Requesting you to please take further action on user booking.<br/>Mbr/>Anurag";
	$data = array('to' => '$email', 'sub' => 'Registered on $rdate.', 'msg' => $bodymsg);
	send_email($data);
