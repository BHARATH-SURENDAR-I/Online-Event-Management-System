...
function doLogin()
{
	...
	if (dbNumRows($result) == 1) {
		...
		$errorMessage = 'Invalid username/passsword.';
	}
	return $errorMessage;
}
...
function getBookingRecords(){
	...
	$sql 	= "SELECT u.id AS uid,r.id AS rid, u.name, u.phone, u.email,
	...
	while($row = dbFetchAssoc($result)) {
		extract($row);
		$records[] = array("user_id" => $uid,"rid" => $rid, "user_name" => $name,"user_phone" => $phone,"user_email" => $email,"count" => $ucount,"res_date" => $rdate,"status" => $status,"comments" => $comments);	
	}
	return $records;
}
function getUserRecords(){
	...
	while($row = dbFetchAssoc($result)) {
		extract($row);
		$records[] = array("user_id" => $id,"user_name" => $name,"user_phone" => $phone,"user_email" => $email,"type" => $type,"status" => $status,"bdate" => $bdate);	
	}
	return $records;
}
